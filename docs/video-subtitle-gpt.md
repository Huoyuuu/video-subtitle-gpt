...
YTDLP_EXTRA_ARGS=                      # 高级参数，如 --proxy socks5://127.0.0.1:1080
...
bash /home/huoyuuu/video-subtitle-gpt/deploy/install_or_update.sh \
...
```bash
cd /home/huoyuuu/video-subtitle-gpt
nano .env
...
crontab -e
# 每 2 分钟检查更新
*/2 * * * * cd /home/huoyuuu/video-subtitle-gpt && git pull --ff-only && bash scripts/deploy.sh >> /var/log/vsg-deploy.log 2>&1
```
...
```bash
cd /home/huoyuuu/video-subtitle-gpt && git pull --ff-only && bash scripts/deploy.sh
```
